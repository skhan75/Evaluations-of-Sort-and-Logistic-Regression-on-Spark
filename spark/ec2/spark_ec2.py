...
# Ensure disks are unmounted
def umount_disks(node, opts, num_disks):
    for i in range(1, num_disks+1):
        if i == 1:
            umount_command = "if grep '/mnt' /proc/mounts; then sudo umount /mnt; fi"
        else:
            umount_command = "if grep '/mnt%d' /proc/mounts; then sudo umount /mnt%d; fi" % (i, i)
        ssh(node, opts, umount_command)

# Create Raid0
def create_raid0(main_nodes, subordinate_nodes, opts, num_disks):
    if opts.instance_type == "c3.large":
        disk_command = "yes | sudo mdadm --create --verbose /dev/md0 --level=0 --name=MY_RAID --raid-devices=2 /dev/xvdb /dev/xvdc; sudo mkfs.ext4 -L MY_RAID /dev/md0; sudo mkdir -p /mnt; sudo chmod -R 777 /mnt; sudo mount LABEL=MY_RAID /mnt"
    elif opts.instance_type == "d2.xlarge":
        disk_command = "sudo mdadm --create --verbose /dev/md0 --level=0 --name=MY_RAID --raid-devices=3 /dev/xvdb /dev/xvdc /dev/xvdd; sudo mkfs.ext4 -L MY_RAID /dev/md0; sudo mkdir -p /mnt; sudo chmod -R 777 /mnt; sudo mount LABEL=MY_RAID /mnt"
    elif opts.instance_type == "m3.xlarge":
        # Specifically for m3.large
        disk_command = "sudo umount /dev/xvdb; sudo mkfs.ext4 -L MY_RAID /dev/xvdb; sudo mkdir -p /mnt; sudo chmod -R 777 /mnt; sudo mount LABEL=MY_RAID /mnt"

    main = get_dns_name(main_nodes[0], opts.private_ips)

    umount_disks(main, opts, num_disks)
    ssh(main, opts, disk_command)
    for subordinate_node in subordinate_nodes:
        current_subordinate = get_dns_name(subordinate_node, opts.private_ips)
        umount_disks(current_subordinate, opts, num_disks)
        ssh(current_subordinate, opts, disk_command)
...
def deploy_files(conn, root_dir, opts, main_nodes, subordinate_nodes, modules):
    ...
    create_raid0(main_nodes, subordinate_nodes, opts, num_disks)
