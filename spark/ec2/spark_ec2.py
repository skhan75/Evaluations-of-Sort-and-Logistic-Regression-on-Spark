...
# Create Raid0
def create_raid0(master_nodes, slave_nodes, opts, num_disks):
    ...
    umount_disks(master, opts, num_disks)
    ssh(master, opts, disk_command)
    for slave_node in slave_nodes:
        current_slave = get_dns_name(slave_node, opts.private_ips)
        umount_disks(current_slave, opts, num_disks)
        ssh(current_slave, opts, disk_command)
...
def deploy_files(conn, root_dir, opts, master_nodes, slave_nodes, modules):
    ...
    create_raid0(master_nodes, slave_nodes, opts, num_disks)
